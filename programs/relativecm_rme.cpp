...
namespace relcm {
  ...
  void ConstructMu2nNLOOperator(const basis::RelativeCMOperatorParametersLSJT& op_params,
    ...
    double brel = util::brel(oscillator_energy);
    double bcm = util::bcm(oscillator_energy);
    ...
    for (int T = op_params.T0_min; T <= op_params.T0_max; ++T) {
      relcm_sectors[T] = basis::RelativeCMSectorsLSJT(relcm_space, op_params.J0, T, op_params.g0);
    ...
    for (std::size_t sector_index = 0; sector_index < sectors.size(); ++sector_index) {
      const basis::RelativeCMSectorsLSJT::SectorType& sector = sectors.GetSector(sector_index);
...
#pragma omp parallel for collapse(2)
      for (std::size_t bra_index = 0; bra_index < bra_subspace.size(); ++bra_index) {
        for (std::size_t ket_index = 0; ket_index < ket_subspace.size(); ++ket_index) {
          ...
          common_integrand *= ho_wfs.at(bra_lr).row(bra_nr) * ho_wfs.at(ket_lr).row(ket_nr);

          // Reduced matrix element calculation.
          // Pauli matrix tensor product in spin space enforces the bra and ket
          // spins to be the same for the relative-cm part, and to be different
          // for the purely relative part.
          double rme = 0;

          if (bra_S == ket_S) {
            // Relative-cm part.
            double tp_a = am::tp::CCSpinTensorProductRME(bra_state, ket_state, 1, 1, 1, 0, 1);
            tp_a *= -std::sqrt(3.);

            Eigen::ArrayXd y = common_integrand * expmpir;
            y.head(1) = 0; // Required to avoid divide by 0.
            y.tail(1) = 0; // Required to avoid divide by 0.
            double integ_expmpir = quad::spline::Integrate(x, y);

            rme = tp_a * integ_expmpir;

            if (bra_S == 1) { // Rank 2 Pauli Matrix tensor product.
              double tp_b = am::tp::CCSpinTensorProductRME(bra_state, ket_state, 1, 1, 1, 2, 1);
              tp_b *= std::sqrt(3. / 5.);

              double tp_c = am::tp::CCSpinTensorProductRME(bra_state, ket_state, 1, 1, 2, 2, 1);
              tp_c *= std::sqrt(9. / 5.);

              double tp_d = am::tp::CCSpinTensorProductRME(bra_state, ket_state, 3, 1, 2, 2, 1);
              tp_d *= std::sqrt(14. / 5.);

              double tp_e = am::tp::CCSpinTensorProductRME(bra_state, ket_state, 3, 1, 3, 2, 1);
              tp_e *= std::sqrt(28. / 5.);

              y *= wpir;
              y.head(1) = 0; // Required to avoid divide by 0.
              y.tail(1) = 0; // Required to avoid divide by 0.
              double integ_expmpir_wpir = quad::spline::Integrate(x, y);
            ...
            // Purely relative part. The cm labels for the bra and ket must be
            // the same.
            if ((bra_nc == ket_nc) && (bra_lc == ket_lc)) {
              double tp_f = am::tp::CCSpinTensorProductRME(bra_state, ket_state, 2, 0, 2, 1, 1);
              tp_f *= std::sqrt(10.);
              ...
              if (bra_lr == ket_lr) { // Rank 0 spherical harmonic.
                double tp_g = am::tp::CCSpinTensorProductRME(bra_state, ket_state, 0, 0, 0, 1, 1);

                y = common_integrand * tpir * ypir;
                y.head(1) = 0; // Required to avoid divide by 0.
                y.tail(1) = 0; // Required to avoid divide by 0.
                double integ_tpir_ypir = quad::spline::Integrate(x, y);

                rme += tp_g * integ_tpir_ypir;
              }
            }
          }
          rme *= am::tp::SpinTensorProductRME(bra_T, ket_T, 1); // Isospin.
