...
namespace am {
  namespace tp {
    ...
    inline
    double SpinTensorProductRME(const int& sp, const int& s, const int& rank) {
      ...
      if (rank == 0) {
        ...
        return std::sqrt(20. / 3.) * (1 - std::abs(sp - s)) * (1 - std::abs(sp - 1));
      }
    }
    ...
    //
    // Arguments:
    //   bra (basis::RelativeStateLSJT): bra state containing am labels
    //   ket (basis::RelativeStateLSJT): ket state containing am labels
    ...
    //   reduced matrix element
    inline
    double CSpinTensorProductRME(const basis::RelativeStateLSJT& bra,
    ...
    //   reduced matrix element
    inline
    double CCSpinTensorProductRME(const basis::RelativeCMStateLSJT& bra,
      ...
      if (am::AllowedTriangle(bra_lr, a, ket_lr)
        ...
        double result = HatProduct(bra_L, bra_S, ket_J, e, bra_lr, bra_lc, ket_L, c);
        result *= am::Wigner9J(ket_L, ket_S, ket_J, c, d, e, bra_L, bra_S, bra_J);
        result *= am::Wigner9J(ket_lr, ket_lc, ket_L, a, b, c, bra_lr, bra_lc, bra_L);
